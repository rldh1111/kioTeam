...
public class ShoppingDAO {
	...
	public Shopping selectByLoginId(Connection conn, String loginId) throws SQLException {
		...
		try (PreparedStatement pst = conn.prepareStatement(sql)) {
			...
			try (ResultSet rs = pst.executeQuery()) {
				if (rs.next()) {
					shopping = new Shopping(rs.getInt("productId"),rs.getString("productType"), rs.getString("loginId"), rs.getString("productName"), rs.getInt("price"), rs.getTimestamp("wdate").toLocalDateTime(),
							rs.getInt("count"));	
				}
				return shopping;
			}
		...
		private Shopping convProduct(ResultSet rs) throws SQLException{
			Shopping shopping = new Shopping(rs.getInt("productId"), rs.getString("productType"),rs.getString("loginId"), rs.getString("productname"), 
			rs.getInt("price"), rs.getTimestamp("wdate").toLocalDateTime(), rs.getInt("count"));
