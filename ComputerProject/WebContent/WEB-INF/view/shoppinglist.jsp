...
<body>
	<div id="top" class="container">
		...
		<div id="basket">
			<a href="shoppinglist" class="basket">장바구니</a>
		</div>
	</div>
	<div id="header">
			...
			<ul>
				<li class="computer"><a href="#" class="computer_drop">컴퓨터
					...
					<div class="computer_content">
						<a href="productlist?productType=cpu">cpu</a> <a
							href="productlist?productType=램">램</a> <a
							href="productlist?productType=메인보드">메인보드</a> <a
							href="productlist?productType=그래픽카드">그래픽 카드</a> <a
							href="?productType=케이스">케이스</a> <a
							href="productlist?productType=파워">파워</a> <a
							href="productlist?productType=키보드">키보드</a> <a
							href="productlist?productType=마우스">마우스</a> <a
							href="productlist?productType=SSD">SSD</a> <a
							href="productlist?productType=odd">odd</a>
					</div></li>
			</ul>
			<ul>
				<li class="sound"><a href="#" class="sound_drop">사운드</a>
					<div class="sound_content">
						<a href="?productType=헤드셋">헤드셋</a> <a href="?productType=스피커">스피커</a>
						<a href="?productType=마이크">마이크</a> <a href="#">사운드 카드</a>
					</div></li>
			</ul>
			<ul>
				<li class="laptop"><a href="#" class="laptop_drop">노트북</a>
					<div class="laptop_content">
						<a href="?productType=게이밍노트북">게이밍 노트북</a> <a href="초경량노트북">초경량
							노트북</a> <a href="디자인노트북">디자인 노트북</a> <a href="비지니스노트북">비지니스 노트북</a>
					</div></li>
			</ul>
			<ul>
				<li class="storage_device"><a href="#"
					...
					<div class="storage_device_content">
						<a href="?productType=외장하드">외장하드</a> <a href="?productType=외장SSD">외장SSD</a>
						<a href="?productType=NAS">NAS</a> <a href="?productType=USB메모리">USB메모리</a>
					</div></li>
			</ul>
		</div>
				...
				<table style="text-align: center">
					...
					<c:if test="${!shoppingPage.hasProduct()}">
						<tr style="text-align: center">
							<td colspan="4">장바구니에 제품이 없습니다.</td>
						</tr>
					</c:if>
					<c:forEach var="shopping" items="${shoppingPage.spList}">
						<tr>
							<form action="shoppingmodify" method="post">
								<input type="hidden" name="productId" value="${shopping.productId}">
								<td style="text-align: center"><input type="hidden" name="productType" value="${shopping.productType }">${shopping.productType }</td>
								<td style="text-align: center">${shopping.productname }</td> 
								<td style="text-align: center" id="price">${shopping.price }</td>
								<td style="text-align: center"><input
									style="text-align: center;" id="count" name="count" type="number" min="1" max="50"
									 value="${shopping.count }"></td>
								<td><input type="button" onclick="allclear(${shopping.productId})" value="x"></td>
					...
					<c:if test="${shoppingPage.hasProduct()}">
						<tr style="text-align: center">
							<td colspan="6"><c:if test="${shoppingPage.startPage > 5 }">
									<a href="shoppinglist?pageNo=${shoppingPage.startPage - 5  }"
										style="text-align: center">[이전]</a>
								</c:if> <c:forEach var="pNo" begin="${shoppingPage.startPage}"
									end="${shoppingPage.endPage }">
									<a href="shoppinglist?pageNo=${pNo}" style="text-align: center">
										[${pNo}]</a>
						...
						<tr>
							<th colspan="4"><span style="text-align: center;"> 총
									가격 : <fmt:formatNumber value="${sum}" currencyCode="kor"/>원
							</span></th>
							<th>${result}</th>
							<th><input type="submit" value="구매하기"></th>
							</form>
						</tr>
					</c:if>
				</table>
