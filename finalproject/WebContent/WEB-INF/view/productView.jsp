...
<link rel="stylesheet" href="css/productView.css?2">
</head>
<body>
<script>
function insertFunc(userId, productId, count){
	...
	if(r == true){
		alert("추가 완료!");
		location.href = "productType?userId="+userId+"&productId="+productId+"&pageNum=${productPage.currentPage}&count="+count;
	}
}
	...
	<c:forEach var="product" items="${productPage.products}">
		...
		<div id="pageNum">
			...
			<c:if test="${productPage.startPage > 5 }">
					<a href="item?productType=${type}&pageNum=${productPage.startPage - 5  }" class="pageNum">이전</a>
			</c:if> 
			<c:forEach var="pageNum" begin="${productPage.startPage}" end="${productPage.endPage }">
				<a href="item?productType=${type}&pageNum=${pageNum}" class="pageNum"> ${pageNum}</a>
			</c:forEach> 
			<c:if test="${productPage.endPage < productPage.totalPages }">
				<a href="item?productType=${type}&pageNum=${productPage.startPage + 5 }" class="pageNum">다음</a>
			</c:if>
		</div>
		</c:if>
	</div>
