...
<link rel="stylesheet" href="css/searchList.css?1">
</head>
<body>
<script>
function insertFunc(userId, productId, count){
	...
	if(r == true){
		alert("추가 완료!");
		location.href = "searchType?search=${search}&userId="+userId+"&productId="+productId+"&pageNum=${productPage.currentPage}&count="+count
	}
}
	...
	<c:forEach var="product" items="${productPage.products}">
		...
		<div id="pageNum">
			<c:if test="${productPage.startPage > 5 }">
					<a href="search?search=${search}&pageNum=${productPage.startPage - 5  }" class="pageNum">이전</a>
			</c:if> 
			<c:forEach var="pageNum" begin="${productPage.startPage}" end="${productPage.endPage }">
				<a href="search?search=${search}&pageNum=${pageNum}" class="pageNum">${pageNum}</a>
			</c:forEach> 
			<c:if test="${productPage.endPage < productPage.totalPages }">
				<a href="search?search=${search}&pageNum=${productPage.startPage + 5 }" class="pageNum">다음</a>
			</c:if>
		</div>
		</c:if>
		</div>
