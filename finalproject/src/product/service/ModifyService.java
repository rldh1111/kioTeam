	...
	public void modify(ModifyRequest mr) {
		...
		try (Connection conn = ConnectionProvider.getConnection()) {
			try {
				...
				Product product1 = productDao.selectName(conn,mr.getProductName());
				if (product1 != null) {
					if (!product.getProductName().equals(mr.getProductName())) {
						throw new DuplicatieException("이름이 중복됩니다");
					}

				}
				productDao.update(conn,mr.getProductId(), mr.getType(), mr.getProductName(), mr.getProductType(), mr.getPrice(),
